...
**What the original figure plots.** It shows zig-zag trajectory of steepest descent across elongated objective contours.

**What this is trying to convey.** Steepest descent can zig-zag when the objective has strongly unequal curvatures.
...
```{figure} figures/09-optimization/redraw-steepest_decent.png
:alt: Polished version of zig-zag trajectory of steepest descent across elongated objective contours
:width: 65%

The polished version preserves zig-zag trajectory of steepest descent across elongated objective contours. The variables, geometry, and comparison are unchanged; the crop, contrast, scale, and line clarity have been standardized for the web.
```
...
The simplex-method project below is embedded from the canonical
[Project 1 ideas](../assignments/project-01.md) file.

:::{include} ../assignments/project-01.md
:start-after: ## Simplex optimization algorithm
:end-before: ## Classification of molecular structures
:filename: false
:::
...
1. **Optimization-method race.** Ask ChatGPT to implement gradient descent,
   Newton, and one quasi-Newton or conjugate-gradient method for the same
   objective. Establish fair stopping criteria, plot all trajectories, and
   compare function evaluations, gradient evaluations, and convergence. Explain
   the results using curvature rather than simply declaring a winner.

2. **Make gradient descent fail.** Challenge ChatGPT to choose a learning rate
   and starting point for a narrow quadratic valley. Construct cases that
   converge slowly, oscillate, and diverge; derive the relevant stability
   condition; and repair the algorithm with scaling or line search. Explain the
   difference between a poor method and poor parameter choices.

3. **Fit a small chemical model.** Use ChatGPT to propose synthetic data for an
   Arrhenius law, decay model, or simple potential-energy curve. Fit the
   parameters from multiple initial guesses, inspect the objective landscape,
   and determine whether the optimum is unique and well constrained. Audit any
   physical interpretation suggested by ChatGPT against units and limiting
   behavior.
