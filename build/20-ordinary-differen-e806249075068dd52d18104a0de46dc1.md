...
- The general form of an $n$-th order linear differential equation is: $$a_0(x) y + a_1(x) y' + a_2(x) y'' + a_3(x) y''' + \dots + a_n(x) y^{(n)} = b(x)$$
...
- In a **homogeneous** linear differential equation, the drive term is set to zero: $$a_0(x) y + a_1(x) y' + a_2(x) y'' + \dots + a_n(x) y^{(n)} = 0$$
...
- A general solution of the inhomogeneous equation, $y(x)$, can be written as the sum of the homogeneous solution and a *particular solution*, $y_p(x)$, that satisfies the inhomogeneous equation: $$y(x) = y_h(x) + y_p(x)$$
...
- The Wronskian of two functions $y_1(x)$ and $y_2(x)$ is defined as: $$W(y_1, y_2) = y_1y_2' - y_2y_1'$$

- More generally, for $n$ functions, the Wronskian is the determinant: $$W(y_1, \ldots, y_n) = \begin{vmatrix}
  y_1 & y_2 & \cdots & y_n \\
  y_1' & y_2' & \cdots & y_n' \\
...
:::{admonition} Example
Consider $f_1(x) = x$ and $f_2(x) = x^2$. Their Wronskian is: $$W(f_1, f_2) = \begin{vmatrix} x & x^2 \\ 1 & 2x \end{vmatrix} = 2x^2 - x^2 = x^2$$ Since this is non-zero for most $x$, the functions are linearly independent.
:::
...
- First, let's solve the homogeneous equation. In the case where $Q(x) = 0$, the equation simplifies to: $$\frac{dy}{dx} = -P(x) y$$

- We can solve this by *separation of variables*: $$\frac{dy}{y} = -P(x) dx$$

- Integrating both sides gives: $$\ln y = - \int P(x) \, dx + C$$

- Exponentiating both sides yields the general solution: $$y(x) = A e^{-\int P(x)dx}$$ where $A = e^C$ is an arbitrary constant.
...
:::{admonition} Example
Consider the simple case of exponential decay where $P(x) = \kappa$, where $\kappa > 0$. Also assume an initial condition $y(0) = 1$. The equation becomes: $$\frac{dy}{dx} + \kappa y = 0$$ Using the general solution for the homogeneous case, we have: $$y(x) = A e^{-\kappa x}$$ From the initial condition, we can solve for $A$. $y(0) = A = 1$.
:::
...
- We will use an *integrating factor* $I(x)$ to rewrite the equation as a total derivative: $$\frac{d}{dx} \left[ I(x) y(x) \right] = I(x) Q(x)$$

- We can solve this new equation just by integrating both sides. First, we need to find $I(x)$ such that this is true.

- To find $I(x)$, we expand the left-hand side using the product rule: $$\frac{d}{dx} \left[ I(x) y(x) \right] = I(x) \frac{dy}{dx} + y(x) \frac{dI}{dx}$$

- Substitute this into the original equation: $$I(x) \frac{dy}{dx} + y(x) \frac{dI}{dx} = I(x) Q(x)$$


## Inhomogeneous Equation (continued)


- Divide by $I(x)$ to get: $$\frac{dy}{dx} + y(x) \frac{1}{I(x)} \frac{dI}{dx} = Q(x)$$

- This becomes the original ODE as long as: $$\frac{1}{I}  \frac{dI}{dx} = P(x)$$

- which simplifies to: $$\frac{dI}{I} = P(x) \, dx$$

- Integrating both sides gives: $$I(x) =  e^{\int P(x) \, dx}$$
...
- Now that we have the integrating factor $I(x)$, we rewrite the general 1st order inhomogeneous ODE in terms of the total derivative: $$\frac{d}{dx} \left[ I(x) y(x) \right] = I(x) Q(x)$$

- Integrating both sides with respect to $x$ gives: $$I(x) y(x) = \int I(x) Q(x) \, dx + C$$

- Solving for $y(x)$: $$y(x) = \frac{1}{I(x)} \left( \int I(x) Q(x) \, dx + C \right)$$

- Substituting $I(x) = e^{\int P(x) \, dx}$ into the equation: $$y(x) = e^{-\int P(x) dx} \left( \int e^{\int P(x) dx} Q(x) \, dx + C \right)$$
...
:::{admonition} Example
Let's apply this method to the following equation: $$y' + y = e^x$$ Here, $P(x) = 1$ and $Q(x) = e^x$. First, we compute the integrating factor: $$I(x) = e^{\int 1 dx} = e^x$$ Multiplying through by the integrating factor: $$e^x y' + e^x y = e^{2x}$$ $$\Rightarrow \frac{d}{dx} \left( e^x y \right) = e^{2x}$$
:::
...
Integrating both sides: $$e^x y = \int e^{2x} dx = \frac{1}{2} e^{2x} + C$$ Finally, solving for $y(x)$: $$y(x) = e^{-x} \left( \frac{1}{2} e^{2x} + C \right) = \frac{1}{2} e^x + C e^{-x}$$
:::
