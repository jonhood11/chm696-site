...
**What the original figure plots.** It shows matrix-vector multiplication shown as row-by-column dot products.

**What this is trying to convey.** Matrix-vector multiplication.
...
```{figure} figures/04-vector-spaces-and-matrices/redraw-vec_mat_mult.png
:alt: Polished version of matrix-vector multiplication shown as row-by-column dot products
:width: 65%

The polished version preserves matrix-vector multiplication shown as row-by-column dot products. The variables, geometry, and comparison are unchanged; the crop, contrast, scale, and line clarity have been standardized for the web.
```

A particular component: $y_2 = A_{21}x_1 + A_{22}x_2 + \cdots + A_{2N}x_N$


**What the original figure plots.** It shows construction of a selected component in a matrix-vector product.

**What this is trying to convey.** A selected component of a matrix-vector product.
...
```{figure} figures/04-vector-spaces-and-matrices/redraw-vec_mat_mult2.png
:alt: Polished version of construction of a selected component in a matrix-vector product
:width: 65%

The polished version preserves construction of a selected component in a matrix-vector product. The variables, geometry, and comparison are unchanged; the crop, contrast, scale, and line clarity have been standardized for the web.
```
...
**What the original figure plots.** It shows row-by-column construction of a matrix-matrix product.

**What this is trying to convey.** Each entry of a matrix product is a row-column dot product.
...
```{figure} figures/04-vector-spaces-and-matrices/redraw-mat_mat_mult.png
:alt: Polished version of row-by-column construction of a matrix-matrix product
:width: 70%

The polished version preserves row-by-column construction of a matrix-matrix product. The variables, geometry, and comparison are unchanged; the crop, contrast, scale, and line clarity have been standardized for the web.
```
...
These optional figures are placed after the lecture so they can be evaluated
without interrupting the current organization.
...
The same act of taking linear combinations produces a line from one direction
and a plane from two independent directions. This connects span directly to
dimension.
...
v = np.array([1.0, 0.55, 0.3])
line = t[:, None] * v
ax1.plot(*line.T, color=DARK_GOLD, linewidth=3)
ax1.quiver(0, 0, 0, *v, color=BLACK, arrow_length_ratio=0.12)
ax1.set(title="One independent vector spans a line",
        xlabel="$x$", ylabel="$y$", zlabel="$z$")

ax2 = fig.add_subplot(1, 2, 2, projection="3d")
v1 = np.array([1.0, 0.1, 0.25])
v2 = np.array([0.15, 0.9, 0.35])
s, t2 = np.meshgrid(np.linspace(-1.3, 1.3, 12), np.linspace(-1.3, 1.3, 12))
plane = s[...,None]*v1 + t2[...,None]*v2
ax2.plot_surface(plane[:,:,0], plane[:,:,1], plane[:,:,2],
                 color=GOLD, alpha=0.42, edgecolor=GRAY, linewidth=0.3)
for vec, color in [(v1, BLACK), (v2, DARK_GOLD)]:
    ax2.quiver(0, 0, 0, *vec, color=color, linewidth=2, arrow_length_ratio=0.12)
ax2.set(title="Two independent vectors span a plane",
        xlabel="$x$", ylabel="$y$", zlabel="$z$")
fig.tight_layout()
plt.show()
```

### A matrix maps the unit circle to an ellipse

The image of a circle shows, in one picture, how a linear map stretches,
rotates, and mixes directions while still preserving straight lines.

```{code-cell} python
A = np.array([[1.7, 0.8], [0.25, 0.75]])
theta = np.linspace(0, 2*np.pi, 500)
circle = np.vstack([np.cos(theta), np.sin(theta)])
ellipse = A @ circle

fig, axes = plt.subplots(1, 2, figsize=(9.4, 4.3))
for ax, points, title in [(axes[0], circle, "input vectors"),
                          (axes[1], ellipse, "output vectors A x")]:
    ax.plot(points[0], points[1], color=DARK_GOLD, linewidth=2.5)
    for angle in np.linspace(0, 2*np.pi, 12, endpoint=False):
        source = np.array([np.cos(angle), np.sin(angle)])
        endpoint = source if ax is axes[0] else A @ source
        ax.plot([0, endpoint[0]], [0, endpoint[1]], color=GRAY, alpha=0.5)
    ax.axhline(0, color=BLACK, linewidth=0.8)
    ax.axvline(0, color=BLACK, linewidth=0.8)
    ax.set(aspect="equal", xlim=(-2.7,2.7), ylim=(-2.1,2.1),
           xlabel="$x_1$", ylabel="$x_2$", title=title)
fig.suptitle("A linear transformation acts on every direction at once")
fig.tight_layout()
plt.show()
```
...
The three panels enforce the order of operations: $B$ acts first, followed by
$A$. The product $AB$ is the single transformation with the same final result.

```{code-cell} python
A = np.array([[1.0, -0.45], [0.35, 1.0]])
B = np.array([[1.5, 0.0], [0.0, 0.65]])
shape = np.array([[0,0], [1.6,0], [0.4,1.2]])
stages = [shape, shape @ B.T, shape @ B.T @ A.T]
titles = ["x", "B x", "A(B x) = (AB)x"]

fig, axes = plt.subplots(1, 3, figsize=(11, 3.9), sharex=True, sharey=True)
for ax, vertices, title in zip(axes, stages, titles):
    ax.add_patch(Polygon(vertices, closed=True, facecolor=GOLD,
                         edgecolor=DARK_GOLD, linewidth=2, alpha=0.6))
    ax.scatter(vertices[:,0], vertices[:,1], color=BLACK, s=28, zorder=3)
    ax.axhline(0, color=GRAY, linewidth=0.8)
    ax.axvline(0, color=GRAY, linewidth=0.8)
    ax.set(aspect="equal", xlim=(-1,2.8), ylim=(-0.5,2.2), title=title)
fig.suptitle("The rightmost matrix acts first")
fig.tight_layout()
plt.show()
```

### One vector in two orthonormal bases

The physical vector does not change when the basis changes; only its component
column changes. This distinguishes a vector from its coordinates.
...
for ax, basis, comps, title in [
    (axes[0], np.eye(2), v, rf"standard components = ({v[0]:.1f}, {v[1]:.1f})"),
    (axes[1], Q, coordinates,
     rf"rotated-basis components = ({coordinates[0]:.2f}, {coordinates[1]:.2f})"),
]:
    ax.quiver(0, 0, *v, angles="xy", scale_units="xy", scale=1,
              color=BLACK, width=0.014)
    for j, color in zip(range(2), [DARK_GOLD, GOLD]):
        ax.quiver(0, 0, *basis[:,j], angles="xy", scale_units="xy", scale=1,
                  color=color, width=0.012)
        ax.text(*(1.18*basis[:,j]), f"e{j+1}'" if ax is axes[1] else f"e{j+1}")
    ax.set(aspect="equal", xlim=(-1.2,3.6), ylim=(-0.8,3.2),
           xlabel="$x$", ylabel="$y$", title=title)
fig.suptitle("The arrow is unchanged; its coordinates depend on the basis")
fig.tight_layout()
plt.show()
```
...
The archived problem sets do not isolate vector spaces and basic matrix
representations as a separate assignment. The complete source sets remain in
[Assignments and projects](../assignments/index.md). Jonathan's original
linearity-classification problem is included below.

:::{include} ../assignments/jonathan-homework-pool.md
:start-after: ## Linear-operator classification
:end-before: ## Complex waves and optical intensity
:filename: false
:::
...
1. **Vector-space courtroom.** Ask ChatGPT to generate unusual proposed vector
   spaces: polynomials with restrictions, probability vectors, functions,
   matrices, or sets with modified addition. For three examples, either verify
   the axioms or identify the first axiom that fails with a concrete
   counterexample. Explain why closure alone is not enough.

2. **Same object, different coordinates.** Represent one polynomial, function,
   or physical vector in two different bases. Ask ChatGPT to derive the
   change-of-basis matrix, then verify it by reconstructing the same object from
   both component columns. Include a diagram or numerical check showing that
   the object is unchanged although its coordinates differ.

3. **Linear-transformation detective.** Give ChatGPT several input-output pairs
   and ask it to infer a matrix. Decide which observations are sufficient for a
   unique answer, reconstruct the matrix when possible, and create one nonlinear
   impostor that matches the supplied examples but fails linearity elsewhere.
   Explain why testing only a few vectors can be misleading.
