...
## Solve Integral: $ \int_-\infty^\infty \frac\cos x1+x^2 dx$
...
- Evaluate inverse Fourier transform for 
$$
F(s)=\dfrac{1}{s(s+1)}
$$
...
$$
0 = \oint_C \frac{f(z)}{z-a} dz = i f(a) \int_{-\pi}^0 d\theta + \mathrm{P.V.}\int_{-\infty}^{\infty} \frac{f(x)}{x-a} dx,
$$
...
We have: 
$$
\mathrm{P.V.}\int_{-\infty}^{\infty} \frac{F(\omega')}{\omega' - \omega} dx = i \pi F(\omega)
$$
