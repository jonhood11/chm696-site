...
- Economy SVD (still exact!): 
$$
A = U\Sigma V^T = \sigma_1 u_1v_1^T + \sigma_2 u_2v_2^T + \dots + \sigma_m u_mv_m^T = \hat{U}\hat{\Sigma}V^T,
$$
 where $\hat{U}$ is nxm and $\hat{\Sigma}$ is mxm matrices.
