...
- Consider a second-order linear ordinary differential equation (ODE) of the form: $$\begin{equation}
  \label{eq:sl_basic}
  \frac{d^2 \psi(x)}{dx^2} + K^2 \psi(x) = 0.
  \end{equation}$$

- This equation can be rewritten in operator form as: $$\begin{equation}
  \mathcal{L} \psi = K^2 \psi,
  \end{equation}$$ where $\mathcal{L}$ is the differential operator defined by: $$\begin{equation}
  \mathcal{L} = -\frac{d^2}{dx^2}.
  \end{equation}$$

- This formulation leads to the general eigenvalue problem: $$\begin{equation}
  \mathcal{L} \psi = \lambda \psi,
  \end{equation}$$ with boundary conditions $\psi(0) = \psi(a) = 0$.
...
- The boundary conditions require the solution to be represented as a Fourier sine series: $$\begin{equation}
  \psi_n(x) = \sqrt{\frac{2}{a}} \sin\left( \frac{n \pi x}{a} \right), \quad K = \frac{n \pi}{a},
  \end{equation}$$ where $n$ is a positive integer.
...
- We can manually verify that these eigenfunctions $\psi_n(x)$ are orthogonal under the inner product: $$\begin{equation}
...
- We now introduce a more general notation for the inner product, which is not yet specific to quantum mechanics. We define: $$\begin{equation}
      \langle \psi_n | \psi_m \rangle = \int \psi_n(x)^* \psi_m(x) \, dx = \delta_{nm},
  \end{equation}$$ where the Kronecker delta $\delta_{nm}$ is defined as $$\begin{equation}
      \delta_{nm} =
      \begin{cases}
          1 & \text{if } n = m, \\
          0 & \text{if } n \neq m.
...
- We also introduce the notation for the inner product involving an operator $\mathcal{A}$: $$\begin{equation}
      \langle \psi_n | \mathcal{A} | \psi_m \rangle = \int \psi_n(x)^* \mathcal{A} \psi_m(x) \, dx.
  \end{equation}$$ In quantum mechanics, this represents the expectation value of the operator $\mathcal{A}$. However, in this mathematical context, it can be interpreted as an average of the operator over the functions.


## Dirac notations for inner product (continued)


- For example, for the operator $\mathcal{L}$, we have: $$\begin{equation}
  \int_0^a \psi_n(x) \mathcal{L} \psi_m(x) \, dx = \lambda_m \int_0^a \psi_n(x)^* \psi_m(x) \, dx,
  \end{equation}$$ which can be expressed using our new notation as: $$\begin{equation}
  \langle \psi_n | \mathcal{L} | \psi_m \rangle = \lambda_m \langle \psi_n | \psi_m \rangle.
  \end{equation}$$

- Additionally, the operator can act on the first function in the inner product: $$\begin{equation}
...
- Similarly, $$\begin{equation}
  ...
  \end{equation}$$ which highlights the linearity of the inner product operation.


## Hermitian operators


- An operator $\mathcal{L}$ is called **Hermitian** (or **self-adjoint**) if it satisfies: $$\begin{equation}
      \langle v | \mathcal{L} u \rangle = \langle \mathcal{L} v | u \rangle
  \end{equation}$$ for all functions $u$ and $v$ in the domain of $\mathcal{L}$, given appropriate boundary conditions.

- For the operator to be Hermitian, the following condition must hold: $$\begin{equation}
      \int_a^b v^*(x) \mathcal{L} u(x) \, dx = \int_a^b \left[ \mathcal{L} v(x) \right]^* u(x) \, dx
  \end{equation}$$

- Given the eigenvalue equations: $\mathcal{L} u = \lambda_u u, \quad \mathcal{L} v = \lambda_v v$, and assuming $\mathcal{L}$ is self-adjoint with $\lambda_u \neq \lambda_v$, we have: $$\begin{equation}
      (\lambda_u - \lambda_v) \int_a^b v^*(x) u(x) \, dx = 0.
  \end{equation}$$

- Thus, the eigenfunctions corresponding to different eigenvalues are orthogonal: $$\int_a^b v^*(x) u(x) \, dx = 0 \quad \text{if} \quad \lambda_u \neq \lambda_v$$


## Proof of self-adjointness


- A general second-order linear differential operator can be written as: $$\begin{equation}
      \mathcal{L} u = \frac{d}{dx} \left[ p(x) \frac{du}{dx} \right] + q(x) u,
  \end{equation}$$ where $p(x)$ and $q(x)$ are sufficiently smooth functions on the interval $[a, b]$.
...
- Consider the integral: $$\int_a^b v^*(x) \mathcal{L} u(x) \, dx = \int_a^b v^*(x) \left( \frac{d}{dx} \left[ p(x) \frac{du}{dx} \right] + q(x) u(x) \right) dx$$

- Integrate by parts: $$\int_a^b v^*(x) \mathcal{L} u(x) \, dx$$ $$= \left[ v^*(x) p(x) \frac{du}{dx} \right]_a^b - \int_a^b \left( \frac{d v^*(x)}{dx} \right) p(x) \frac{du}{dx} \, dx + \int_a^b v^*(x) q(x) u(x) \, dx$$

- Similarly, for $\int_a^b \left[ \mathcal{L} v(x) \right]^* u(x) \, dx$, we obtain: $$\int_a^b \left[ \mathcal{L} v(x) \right]^* u(x) \, dx$$ $$= \left[ p(x) \frac{d v^*(x)}{dx} u(x) \right]_a^b - \int_a^b p(x) \frac{d v^*(x)}{dx} \frac{du}{dx} \, dx + \int_a^b v^*(x) q(x) u(x) \, dx$$
...
- Subtracting the two expressions, the difference involves only the boundary terms: $$\int_a^b v^*(x) \mathcal{L} u(x) \, dx - \int_a^b \left[ \mathcal{L} v(x) \right]^* u(x) \, dx$$ $$= \left[ v^*(x) p(x) \frac{du}{dx} - p(x) \frac{d v^*(x)}{dx} u(x) \right]_a^b$$

- For $\mathcal{L}$ to be self-adjoint, the boundary terms must vanish: $$\begin{equation*}
...
- A general form of the Sturm--Liouville equation is: $$p(x)\frac{d^2y}{dx^2} + r(x)\frac{dy}{dx} + q(x)y + \lambda \rho(x)y = 0,$$ where $r(x) = \frac{dp(x)}{dx}$, and $p$, $q$, $r$ are real functions of $x$.

- This can be written as: $$\mathcal{L}y = \lambda \rho(x) y,$$ where $$\mathcal{L} \equiv - \left[  p(x)\frac{d^2y}{dx^2} + r(x)\frac{dy}{dx} + q(x)y \right]$$

- Combining with condition $r(x) = \frac{dp(x)}{dx}$, the general Sturm--Liouville equation can also be rewritten as: $$(py')' + q(x)y + \lambda \rho(x)y = 0$$

- Thus, the Sturm--Liouville linear operator can be defined as $$\mathcal{L} u \equiv -\Biggl[  \frac{d}{dx} \left( p(x) \frac{du}{dx} \right) + q(x) u \Biggr]$$
...
- Any second-order differential equation of the form $$p(x)\frac{d^2y}{dx^2} + r(x)\frac{dy}{dx} + q(x)y + \lambda \rho(x)y = 0$$ can be converted into Sturm--Liouville form by multiplying through by an integrating factor: $$F(x) = \exp \left\{ \int ^x \frac{r(u) - p'(u)}{p(u)} \right\}$$

- Then the original equation takes the Sturm--Liouville form: $$[F(x)p(x)y']' + F(x)q(x)y + \lambda F(x)ρ(x)y = 0$$


## Example: converting the ODE into Sturm-Liouville form


- Consider Hermite equation: $$y'' - 2xy' + 2\nu y = 0$$

- Here, $p = 1$, $r = -2x$, $q = 2\nu$

- The integrating factor is: $$F(x) = \exp \left\{ \int ^x \frac{r(u) - p'(u)}{p(u)} \right\} = \exp\left\{ \int ^x -2u du \right\} = \exp(-x^2)$$

- Thus, the Hermite equation becomes: $$e^{-x^2}y'' -2xe^{-x^2}y' +2 \nu e^{-x^2} y = (e^{-x^2}y')' +2\nu e^{-x^2}y = 0$$

- This equation is in Sturm-Liouville form $$(py')' + q(x)y + \lambda \rho(x)y = 0$$ with $p(x) = e^{-x^2}$, $q(x) = 0$, $\rho(x)= e^{-x^2}$, $\lambda = 2\nu$


## The weight function


- The general SL equation: $$(py')' + q(x)y + \lambda \rho(x)y = 0$$
...
- The inner product is then defined as: $$\langle v |  u \rangle = \int_a^b u^*(x) v(x) \rho(x) dx$$

- Orthogonality condition of the eigenfunctions corresponding to different eigenvalues: $$\int_a^b v^*(x) u(x)\rho(x) \, dx = 0 \quad \text{if} \quad \lambda_u \neq \lambda_v$$
...
- A set of conditions that ensures the SL operator $L$ is Hermitian over the range \[a,b\] is the vanishing of the boundary term: $$\left[ v^*(x) p(x) \frac{du}{dx} - p(x) \frac{d v^*(x)}{dx} u(x) \right]_a^b = 0$$
...
- Legendre's differential equation is: $$\begin{equation}
  (1 - x^2) \frac{d^2 y}{dx^2} - 2 x \frac{dy}{dx} + n(n + 1) y = 0
  \end{equation}$$ where $n$ is a non-negative integer.
...
- Bessel's differential equation is: $$\begin{equation}
...
- Properties:
  ...
  - Orthogonality over specific intervals with weight $w(x) = x$: $$\begin{equation}
...
- Hermite's differential equation is: $$\begin{equation}
...
- Properties:
  ...
  - Orthogonality with weight $w(x) = e^{-x^2}$: $$\begin{equation}
...
- Laguerre's differential equation is: $$\begin{equation}
  x \frac{d^2 y}{dx^2} + (1 - x) \frac{dy}{dx} + n y = 0
  \end{equation}$$ with $n$ being a non-negative integer.
...
- Properties:
  ...
  - Orthogonality with weight $w(x) = e^{-x}$: $$\begin{equation}
