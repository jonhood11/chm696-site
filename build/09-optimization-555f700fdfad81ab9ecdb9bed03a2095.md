...
The system $A \mathbf{x} = \mathbf{b}$ can be recast as a quadratic optimization problem. Define: $$f(\mathbf{x}) = \frac{1}{2} \mathbf{x}^T A \mathbf{x} - \mathbf{x}^T \mathbf{b}.$$
...
Two vectors $\mathbf{p}_i$ and $\mathbf{p}_j$ are [$A$-orthogonal (conjugate)]{style="color: blue"} if: $$\mathbf{p}_i^T A \mathbf{p}_j = 0 \quad \text{for } i \neq j.$$
:::
...
- Iterative Steps
  ...
  1.  Compute the step size: $$\alpha_k = \frac{\mathbf{r}_k^T \mathbf{r}_k}{\mathbf{p}_k^T A \mathbf{p}_k}$$.
  ...
  4.  Compute the conjugacy coefficient: $$\beta_k = \frac{\mathbf{r}_{k+1}^T \mathbf{r}_{k+1}}{\mathbf{r}_k^T \mathbf{r}_k}.$$
...
The CG method minimizes the quadratic function: $$f(\mathbf{x}) = \frac{1}{2} \mathbf{x}^T A \mathbf{x} - \mathbf{b}^T \mathbf{x}.$$

Using the Taylor series expansion: $$f(\mathbf{x} + \Delta \mathbf{x}) = f(\mathbf{x}) + \nabla f(\mathbf{x})^T \Delta \mathbf{x} + \frac{1}{2} \Delta \mathbf{x}^T A \Delta \mathbf{x}.$$
...
- The update becomes $$\mathbf{x}_{k+1} = \mathbf{x}_k - B_k \, \nabla f(\mathbf{x}_k),$$ where $B_k$ is a computationally cheaper approximation of the inverse Hessian. The method uses gradient information from previous iterations to improve $B_k$ step by step.
...
- **BFGS** (Broyden--Fletcher--Goldfarb--Shanno) $$B_{k+1} = \left(I - \frac{\mathbf{s}_k \mathbf{y}_k^T}{\mathbf{y}_k^T \mathbf{s}_k}\right) B_k
...
- **DFP (Davidon--Fletcher--Powell)** $$B_{k+1} = B_k + \frac{\mathbf{s}_k \mathbf{s}_k^T}{\mathbf{s}_k^T \mathbf{y}_k} - \frac{B_k \mathbf{y}_k \mathbf{y}_k^T B_k}{\mathbf{y}_k^T B_k \mathbf{y}_k}$$
