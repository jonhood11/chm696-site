...
- evaluate real integrals and inverse Laplace transforms with residues;
- define and calculate Cauchy principal values;
- track the sign of a real-axis pole indentation; and
- derive Kramers--Kronig relations from causality and analyticity.
...
**What this is trying to convey.** Complex differentiability requires path-independent convergence of the difference quotient.

:::{figure} figures/27-residues-and-dispersion/cf1.png
:alt: Several paths approaching the same point in the complex plane
:width: 75%

Complex differentiability requires path-independent convergence of the
difference quotient.
:::

**Polished version.**

:::{figure} figures/27-residues-and-dispersion/redraw-cf1.png
...
- In $\mathbb{C}$, there are infinitely many approach paths.

Therefore the quotient must have the same limit along every path, not merely
every straight-line direction.
...
- integrals over closed loops vanish when the loop is contractible within the
  holomorphic domain

- uniquely determined on a connected domain by values on a set with an
  accumulation point
...
- We can define the complex function $f(z)$ and the differential $dz$ as:
```{math}
\begin{aligned}
          f(z) &= u(x, y) + iv(x, y) \\
          dz &= dx + i dy
  \end{aligned}
```
 where $u$ and $v$ are real functions of the real variables $x$ and $y$.

- Expanding the complex integral yields a sum of four real line integrals:
```{math}
...
- By setting $P=u$ and $Q=-v$ for the real part, and $P=v$ and $Q=u$ for the imaginary part, the total integral $I$ becomes:
```{math}
\begin{aligned}
          I = \oint_{C} f(z) dz
...
- If $f(z) = u + iv$ is analytic, $u$ and $v$ must satisfy:
```{math}
...
- Thus, under the differentiability hypotheses required by Green's theorem, the
  Cauchy--Riemann equations make both area integrands vanish. This proves this
  form of **Cauchy's theorem** for a simple closed contour and its interior.
...
5. If the contour is positively oriented, its arc contribution vanishes, and
   it contains no pole on the real axis, conclude
```{math}
\int_{-\infty}^{\infty} g(x)\,dx
...
## Example: $\displaystyle\int_{-\infty}^{\infty}
\frac{\cos x}{1+x^2}\,dx$

Solve Integral: $\displaystyle \int_{-\infty}^{\infty} \frac{\cos x}{1+x^2} dx$ We use the function:
```{math}
f(z) = \frac{e^{iz}}{1+z^2}.
```
 Its real part is $\cos x/(1+x^2)$.

Poles occur at:
```{math}
z = i, \qquad z = -i.
```
 We close the contour in the upper half-plane because $e^{iz}$ decays there:
```{math}
|e^{\mathrm{i}z}| = |e^{\mathrm{i}x-y}| = |e^{\mathrm{i}x}||e^{-y}|=|e^{-y}| \le 1
```
...
The residue is
```{math}
\operatorname{Res}(f,i) = \lim_{z\to i} (z-i) \frac{e^{iz}}{(z+i)(z-i)} = \frac{e^{i i}}{2i} = \frac{e^{-1}}{2i}.
```
 Contour integral:
```{math}
\oint f(z) dz = 2\pi i \cdot \frac{e^{-1}}{2i} = \pi e^{-1}.
```
 But:
```{math}
\oint f(z) dz = \int_{-\rho}^{\rho} \frac{e^{ix}}{1+x^2} dx + \int_{\text{along upper half of } z = \rho e^{i\theta}} \frac{e^{iz}}{1+z^2} dz.
```


Take real part:
```{math}
I = \int_{-\infty}^{\infty} \frac{\cos x}{1+x^2} dx = \operatorname{Re}(\pi e^{-1}) = \pi e^{-1}.
```
...
The inverse Laplace transform can be computed using [Bromwich integral]{style="color: blue"}:
```{math}
...
- Evaluate the inverse Laplace transform for
```{math}
F(s)=\dfrac{1}{s(s+1)}
```
...
```{math}
f(t)=\sum_{\text{poles inside}} \operatorname{Res}\big(F(s)e^{st},s_k\big).
```
...
At $s=0$ (simple pole):
```{math}
\operatorname{Res}_{s=0} = \lim_{s\to0} s\frac{e^{st}}{s(s+1)} = \lim_{s\to0}\frac{e^{st}}{s+1} = 1 .
```


At $s=-1$ (simple pole):
```{math}
\operatorname{Res}_{s=-1} = \lim_{s\to-1}(s+1)\frac{e^{st}}{s(s+1)}
...
Therefore
```{math}
f(t) = 2\pi i\!\sum\text{Res}\times\frac{1}{2\pi i}
= 1 - e^{-t}.
```
...
Many integrals in physics and engineering contain singularities on the real axis. Examples:
```{math}
\int_{-\infty}^{\infty} \frac{f(x)}{x-x_0} \, dx, \qquad
\int_0^{\infty} \frac{\sin x}{x} \, dx.
```
 These are not defined in the usual sense. The [Cauchy Principal Value (P.V.)]{style="color: blue"} provides a consistent interpretation.
...
For a function with a simple pole at $x=a$, the [principal value]{style="color: blue"} integral is defined as:
```{math}
...
Suppose $f(z)$ has a simple pole at $z=a$:
```{math}
f(z) \sim \frac{\operatorname{Res}(f,a)}{z-a}.
```


Parameterize the indentation:
```{math}
...
Substitute:
```{math}
\int_{C_\varepsilon} f(z)dz \approx \int \frac{\operatorname{Res}(f,a)}{\varepsilon e^{i\theta}} i\varepsilon e^{i\theta} d\theta = i\,\operatorname{Res}(f,a) \int d\theta.
```
...
Consider the integral
```{math}
\int_{-\infty}^{\infty} \frac{f(x)}{x-a} dx,
```
 in which $f(z)$ is analytic for $y\ge 0$ and $\rightarrow 0$ at $\infty$; $a$ is real.

Then we can write a contour identity:


```{math}
0 = \oint_C \frac{f(z)}{z-a} dz
= i f(a)\int_{\pi}^{0}d\theta
+\mathrm{P.V.}\int_{-\infty}^{\infty}\frac{f(x)}{x-a}\,dx,
```
 for the contour that avoids the pole with a small semicircle.

Thus,
```{math}
\mathrm{P.V.}\int_{-\infty}^{\infty} \frac{f(x)}{x-a} dx = i \pi f(a)
```
...
Consider a complex response function:
```{math}
F(\omega) = F_R(\omega) + i F_I(\omega),
```
 analytic in the upper half-plane.

For a simple contour that avoids a pole on the real axis:
```{math}
\oint \frac{F(z)}{z-\omega} dz = 0.
```
...
Splitting real and imaginary parts gives [Kramers--Kronig relations]{style="color: blue"}:
```{math}
\mathrm{P.V.}\int_{-\infty}^{\infty} \frac{F_R(\omega')}{\omega' - \omega} d\omega' = - \pi F_I(\omega)
```

```{math}
\mathrm{P.V.}\int_{-\infty}^{\infty} \frac{F_I(\omega')}{\omega' - \omega} d\omega' = \pi F_R(\omega)
```
...
These responses are encoded in the **complex susceptibility**
```{math}
...
Consider a damped driven mechanical oscillator:
```{math}
...
```{math}
x(\omega) = \chi(\omega) F_0,
```
 where the susceptibility is
```{math}
\chi(\omega) = \frac{1}{k - m\omega^2 - i b\omega}.
```
...
Write
```{math}
\chi(\omega) = \frac{k - m\omega^2}{(k - m\omega^2)^2 + (b\omega)^2}
...
\chi'(\omega) = \frac{k - m\omega^2}{(k - m\omega^2)^2 + (b\omega)^2}
```
 controls **elastic stiffness**,
```{math}
\chi''(\omega) = \frac{b\omega}{(k - m\omega^2)^2 + (b\omega)^2}
```
 measures **dissipation**.
...
- Changing $b$ changes both the height and width of the loss peak; $\chi''$ is
  not necessarily monotone in $b$ at every fixed frequency.

- KK relation implies a required change in the elastic response $\chi'$.

- For a fixed causal response function, its dispersive and absorptive parts
  cannot be specified independently; they are Hilbert-transform pairs.
