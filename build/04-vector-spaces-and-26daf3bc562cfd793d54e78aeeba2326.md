...
- Inverse elements of vector addition: For every $\mathbf{a}\in V$, there exists an element $-\mathbf{a}\in V$, called the *additive inverse* of $\mathbf{a}$, such that $\mathbf{a}+ (-.\mathbf{a}) = \mathbf{0}$.
...
- Note, however, that: $\langle \lambda \mathbf{a}+ \mu \mathbf{b}|\mathbf{c}\rangle = \lambda^* \langle \mathbf{a}| \mathbf{b}\rangle + \mu^* \langle \mathbf{a}|\mathbf{c}\rangle$
...
Here we assume $\langle \mathbf{a}| \mathbf{a}\rangle \geq 0$, i.e., the linear vector space with *positive semi-definite norm* --- the typical case of our concern.

- Schwarz's inequality: $$|\langle \mathbf{a}| \mathbf{b}\rangle| \leq ||\mathbf{a}|| \: || \mathbf{b}||$$

  Note: the equality holds when $\mathbf{a}= \lambda \mathbf{b}$

- The triangle inequality: $$||\mathbf{a}+ \mathbf{b}|| \leq ||\mathbf{a}|| + ||\mathbf{b}||$$
...
- If a linear operator $\mathcal{A}$ transforms vectors from an N-dimensional vector space (with basis $\mathbf{e}_i$, i=1,2,\..., N) into vectors belonging to an M-dimensional vector space (with basis $\mathbf{f}_i$, i=1,2,\..., M), it can be represented by the $M \times N$ matrix: $$A = \begin{pmatrix}
...
- We may denote a vector $\mathbf{x}$ in terms of its components $x_i$ in a basis $\mathbf{e}_i$, i = 1,2,\..., N, as $$\text{x} = \begin{pmatrix}
  x_1 \\
  x_1 \\
  \vdots \\
  x_N
...
- Addition of matrices: $\mathcal{A} = \mathcal{A}+\mathcal{B}$ can be computed as $S_{ij} = A_{ij} + B_{ij}$

- Matrices must have the same dimensions!

- Matrix addition is commutative and associative.

- Matrix subtraction: similar, i.e., $\mathcal{A} = \mathcal{A}-\mathcal{B} \leftrightarrow S_{ij} = A_{ij} - B_{ij}$

- Multiplication by a scalar is distributive and associative: $$\lambda
  \begin{pmatrix}
  A_{11} & A_{12} &  A_{13}\\
  A_{21} & A_{22} &  A_{23}
  \end{pmatrix}
...
The original lecture refers to `vec_mat_mult.png`. The file is preserved in the private Overleaf archive and will be redrawn or published after its reuse rights are verified.
:::
...
:::{note} Original-slide figure pending review
The original lecture refers to `vec_mat_mult2.png`. The file is preserved in the private Overleaf archive and will be redrawn or published after its reuse rights are verified.
:::
...
- Since $\mathbf{x}$ is arbitrary, $(\text{A}\text{B})_{ij} = A_{ik} B_{kj}$

- Then $\text{P} = \text{AB} \leftrightarrow P_{ij} = \sum_{k=1}^N A_{ik} B_{kj}$ for i = 1,2,\...,M, j= 1,2,\...,R


:::{note} Original-slide figure pending review
The original lecture refers to `mat_mat_mult.png`. The file is preserved in the private Overleaf archive and will be redrawn or published after its reuse rights are verified.
:::

## Multiplication of matrices - cont


- Check the dimensions! For valid $\text{P} = \text{AB}$, $\text{A}$ and $\text{B}$ should be $M \times N$ and $N \times M$ matrices!

- $\text{P} = \text{AB}$ and $\text{Q} = \text{BA}$ are different matrices! ($\text{P}$ is $M \times M$ and $\text{Q}$ is $N \times N$)
...
- The *null* or *zero* matrix $\text{0}$ has all elements equal to zero: $$\text{A0} = \text{0} = \text{0A}$$ $$\text{A+0} = \text{0} = \text{0+A}$$

- The *identity* matrix I has the property $$\text{AI} = \text{IA} = \text{A}$$

- The identity matrix is a square $N \times N$ matrix: $$\text{I}_N =
  \begin{pmatrix}
  1 & 0 & \dots & 0\\
  0 & 1 & \dots & 0\\
  \vdots & \vdots & \ddots & \vdots\\
