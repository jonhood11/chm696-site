...
- Recall from the last lecture: $$\sin\theta = \theta - \theta^3/3! + \theta^5/5! + \cdots$$ $$\cos\theta = 1 - \theta^2/2! + \theta^4/4! + \cdots$$

- Now write the series for $e^{\mathrm{i}\theta}$ for real $\theta$: $$\begin{align*}
...
$$z_1=r_1 e^{\mathrm{i}\theta_1},\quad z_2=r_2 e^{\mathrm{i}\theta_2}$$ $$z_1 z_2 = r_1 r_2 e^{\mathrm{i}(\theta_1+\theta_2)},\qquad
...
**De Moivre's theorem:** $$e^{\mathrm{i}\theta n} = (\cos\theta+\mathrm{i}\sin\theta)^n = \cos(n\theta)+\mathrm{i}\sin(n\theta).$$
...
For $z=re^{\mathrm{i}\theta}$, solutions to $w^n=z$ are $$w_k = r^{1/n} e^{\mathrm{i}(\frac{\theta+2\pi k}{n})},\qquad k=0,1,\dots,n-1.$$ Roots are equally spaced around a circle radius $r^{1/n}$.
...
We can write a pair of equations: $$e^{\mathrm{i}\theta} =  \cos \theta + \mathrm{i}\sin \theta$$ $$e^{- \mathrm{i}\theta} =  \cos \theta - \mathrm{i}\sin \theta$$

And solve them for $\sin$ and $\cos$: $$\sin \theta = \frac{e^{\mathrm{i}\theta} - e^{- \mathrm{i}\theta}}{2 \mathrm{i}}$$ $$\cos \theta = \frac{e^{\mathrm{i}\theta} + e^{- \mathrm{i}\theta}}{2}$$


## Hyperbolic functions


Trigonometric formulas can be generalized for complex numbers z as: $$\sin z = \frac{e^{\mathrm{i}z} - e^{- \mathrm{i}z}}{2 \mathrm{i}}$$ $$\cos z = \frac{e^{\mathrm{i}z} + e^{- \mathrm{i}z}}{2}$$

Rewriting them for pure imaginary $z = \mathrm{i}y$ gives: $$\sin \mathrm{i}y = \frac{e^{-y} - e^{y}}{2 \mathrm{i}} =\mathrm{i}\frac{e^{y} - e^{-y}}{2 } \equiv \sinh y$$ $$\cos \mathrm{i}y = \frac{e^{-y} + e^{y}}{2 \mathrm{i}} = \frac{e^{y} + e^{-y}}{2 } \equiv \cosh y$$

These functions are called **hyperbolic sine** $\sinh$ and **hyperbolic cosine** $\cosh$. Their definitions for all z: $$\sinh z =  \frac{e^{z} - e^{-z}}{2 }$$ $$\cosh z =  = \frac{e^{z} + e^{-z}}{2 }$$
...
$$i \hbar \frac{\partial \psi}{\partial t} = \hat{H} \psi.$$ A free particle solution: $\psi(x,t) = A e^{i(kx - \omega t)}$
