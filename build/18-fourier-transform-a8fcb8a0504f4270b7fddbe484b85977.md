...
The complex form of the Fourier series is given by: $$f(x) = \sum_{k=-\infty}^{\infty} C_k e^{i k\omega x}$$

The complex coefficients $C_n$ are determined by: $$C_k = \frac{1}{2L} \int_{-L}^L f(x) e^{-ik\omega x} \, dx$$

The complex exponentials are orthogonal: $$\frac{1}{2L} \int_{-L}^L e^{im\omega x} e^{-in\omega x} \, dx = \delta_{mn}.$$
...
A normalized Gaussian distribution $f(t)$, centered on $t=0$: $$\begin{equation*}
    f(t) = \frac{1}{\tau \sqrt{2\pi}} \exp \left( - \frac{t^2}{2\tau^2} \right), \quad -\infty < t < \infty
\end{equation*}$$ A root mean square deviation (or \"spread\") in time is $\Delta t =  \tau$.

The Fourier transform $\hat{f}(\omega)$ of $f(t)$: $$\begin{equation*}
...
A critical relationship exists between the spread in the time domain ($\Delta t = \tau$) and the spread in the frequency domain ($\Delta \omega = 1/\tau$). This inverse relationship is a form of the **uncertainty principle**: $$\begin{equation*}
    \Delta \omega \Delta t = 1
\end{equation*}$$ This means that the narrower a signal is in time (small $\Delta t$), the greater the spread of frequency components it must contain (large $\Delta \omega$).
...
The continuous convolution of $f(t)$ and $g(t)$ is defined as: $$(f * g)(t) = \int_{-\infty}^{\infty} f(\tau) g(t - \tau) \, d\tau$$

The convolution is commutative, $$(f * g)(t) = (g * f)(t)$$ The proof for commutativity comes from a quick change of variables by setting $u = t - \tau$, which implies $du = -d\tau$. Substituting into the integral: $$(f * g)(t) = \int_{\infty}^{-\infty} f(t - u) g(u) (-du)$$ Flipping the limits of integration to absorb the negative sign: $$(f * g)(t) = \int_{-\infty}^{\infty} f(t - u) g(u) \, du$$ This is exactly the form of $(g * f)(t)$
...
The Convolution Theorem is a fundamental principle in signal processing and Fourier analysis. It states that under suitable conditions, the Fourier transform of the convolution of two functions is equal to the product of their Fourier transforms. $$\begin{equation}
...
&= F(\omega) G(\omega)
\end{align*}$$ In the second-to-last line, we used the change of variables $u = t - \tau$.
...
The total energy in a signal over all time is given by: $$E = \int_{-\infty}^{\infty} |x(t)|^2 \, dt$$

We can also analyze signals in the frequency domain using the Fourier transform: $X(\omega) = \mathcal{F}\{ x(t) \}$

Parseval's theorem relates the energy of a signal in the time domain to its energy in the frequency domain: $$\int_{-\infty}^{\infty} |x(t)|^2 \, dt = \frac{1}{2\pi}\int_{-\infty}^{\infty} |X(\omega)|^2 \, d\omega$$
...
Parseval's Theorem: $$\int_{-\infty}^{\infty} |x(t)|^2 \, dt = \frac{1}{2\pi}\int_{-\infty}^{\infty} |X(\omega)|^2 \, d\omega$$
...
Start with the inverse Fourier transform: $$\begin{align*}
f(t) &= \frac{1}{2\pi}\int_{-\infty}^{\infty} F(\omega)e^{i\omega t} \, d\omega
\end{align*}$$ To find $|f(t)|^2$, we multiply $f(t)$ by its complex conjugate: $$\begin{align*}
|f(t)|^2 &= f(t)f^*(t) \\
&= \frac{1}{(2\pi)^2}\int_{-\infty}^{\infty}\int_{-\infty}^{\infty} F(\omega)F^*(\omega')e^{i(\omega-\omega')t} \, d\omega \, d\omega'
\end{align*}$$ Now, we integrate both sides over all time: $$\begin{align*}
...
\end{align*}$$ The integral in parentheses is the Dirac delta function: $$\begin{align*}
...
Substituting this back into our equation: $$\begin{align*}
...
\end{align*}$$ This completes the proof of Parseval's Theorem.
...
The discrete Fourier transform is given by: $$\hat{f}_k = \sum_{j=0}^{n-1} f_j e^{-i 2\pi jk /n}$$

The inverse discrete Fourier transform: $$f_k = \frac{1}{n} \sum_{j=0}^{n-1} \hat{f}_j e^{i 2\pi jk /n}$$
...
Thus, DFT is a linear operator (i.e., a matrix) that maps the data points in $\mathbf{f}$ to the frequency domain $\hat{\mathbf{f}}$: $$\{f_0, f_1, f_2, \dots, f_{n-1}  \} \Rightarrow \{\hat{f}_0, \hat{f}_1, \hat{f}_2, \dots, \hat{f}_{n-1} \}$$
...
- Then, the DFT matrix $\mathbf{F}_{1024}$ can be factored out as: $$\mathbf{\hat{f}} = \mathbf{F}_{1024} \mathbf{f} =
      \begin{bmatrix}
