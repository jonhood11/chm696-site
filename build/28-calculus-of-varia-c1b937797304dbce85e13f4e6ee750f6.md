# Calculus of Variations

:::{admonition} Course-material provenance
:class: dropdown
This page was migrated from the CHM 696 course materials developed by Jonathan Hood and Lyudmila Slipchenko. Equations and examples remain under editorial review during the Fall 2026 website migration.
:::

## Variational calculus


- Variational calculus is a branch of mathematical analysis that deals with finding extrema (minimum or maximum values) of functionals, which are mappings from a set of functions to the real numbers. Unlike standard calculus, which focuses on functions of real variables, variational calculus focuses on functionals that depend on functions and their derivatives.

- A **functional** is a rule that assigns a real number to a function. A common form of a functional is: $$J[y] = \int_{a}^{b} L(x, y(x), y'(x)) \, dx$$ where:

  - $L(x, y, y')$ is called the **Lagrangian** and is a given function of $x$, $y(x)$, and $y'(x)$.

  - $y(x)$ is the function to be determined, belonging to a suitable class of functions satisfying given boundary conditions.

  - $y'(x) = \frac{dy}{dx}$ is the derivative of $y(x)$ with respect to $x$.


## The Variational Problem

Functional: $$J[y] = \int_{a}^{b} L(x, y(x), y'(x)) \, dx$$

The fundamental problem in variational calculus is to find the function $y(x)$ that makes the functional $J[y]$ stationary (i.e., achieves an extremum value). This means finding $y(x)$ such that a small variation $\delta y(x)$ around it does not change $J[y]$ to first order:

$$\delta J = J[y + \delta y] - J[y] = 0$$


## The Euler-Lagrange Equation

To find the function $y(x)$ that makes $J[y]$ stationary, we consider a small variation $\delta y(x)$ that vanishes at the endpoints ($\delta y(a) = \delta y(b) = 0$) and compute the first variation $\delta J$:

$$\delta J = \int_{a}^{b} \left[ \frac{\partial L}{\partial y} \delta y + \frac{\partial L}{\partial y'} \delta y' \right] dx$$

Using integration by parts on the second term:

$$\int_{a}^{b} \frac{\partial L}{\partial y'} \delta y' \, dx = \left[ \frac{\partial L}{\partial y'} \delta y \right]_a^b - \int_{a}^{b} \frac{d}{dx} \left( \frac{\partial L}{\partial y'} \right) \delta y \, dx$$

Since $\delta y(a) = \delta y(b) = 0$, the boundary term vanishes, and we have:

$$\delta J = \int_{a}^{b} \left( \frac{\partial L}{\partial y} - \frac{d}{dx} \left( \frac{\partial L}{\partial y'} \right) \right) \delta y \, dx$$

For $\delta J = 0$ to hold for arbitrary $\delta y$, the integrand must be zero:

$$\frac{\partial L}{\partial y} - \frac{d}{dx} \left( \frac{\partial L}{\partial y'} \right) = 0$$


## The Euler-Lagrange Equation (continued)

For the functional: $$J[y] = \int_{a}^{b} L(x, y(x), y'(x)) \, dx$$

This is the **Euler-Lagrange equation** is: $$\frac{\partial L}{\partial y} - \frac{d}{dx} \left( \frac{\partial L}{\partial y'} \right) = 0$$

The **Euler-Lagrange equation** provides a necessary condition for $y(x)$ to be an extremal function for $J[y]$.


## First Integrals of the Euler-Lagrange Equation


- If $L$ does not depend explicitly on $y$: $$\frac{\partial L}{\partial y'} = \text{constant}$$

- If $L$ does not depend explicitly on $x$, change variables: $$x' = \frac{dx}{dy} = \left( \frac{dy}{dx} \right)^{-1}, \; \;  y' = \frac{1}{x'}, \; \; dx =\frac{dx}{dy} dy = x' dy$$

  Then the integrand is a function of $y$ and $x'$, so the Euler equation is now $$\frac{\partial L}{\partial x} - \frac{d}{dy} \left( \frac{\partial L}{\partial x'} \right) = 0$$ and simplifies to $$\frac{\partial L}{\partial x'} = \text{constant}$$


## The Shortest Path (Geodesic) Between Two Points


:::{note}
ProblemFind the function $y(x)$ that minimizes the length of a curve between two points $(x_1, y_1)$ and $(x_2, y_2)$.
:::

The length $S$ of a curve between $x_1$ and $x_2$ is:

$$S[y] = \int_{x_1}^{x_2} \sqrt{1 + \left( y'(x) \right)^2} \, dx$$

Here, the Lagrangian is:

$$L(y, y') = \sqrt{1 + \left( y' \right)^2}$$

Since $L$ does not explicitly depend on $y$, the Euler-Lagrange equation simplifies to:

$$\frac{d}{dx} \left( \frac{\partial L}{\partial y'} \right) = 0$$


## The Shortest Path (Geodesic) Between Two Points (continued)

$$\frac{d}{dx} \left( \frac{\partial L}{\partial y'} \right) = 0$$

Compute $\frac{\partial L}{\partial y'}$:

$$\frac{\partial L}{\partial y'} = \frac{y'}{\sqrt{1 + \left( y' \right)^2}}$$

Set the derivative to zero:

$$\frac{d}{dx} \left( \frac{y'}{\sqrt{1 + \left( y' \right)^2}} \right) = 0$$

This implies that $\frac{y'}{\sqrt{1 + \left( y' \right)^2}} = C$, where $C$ is a constant.


## The Shortest Path (Geodesic) Between Two Points (continued)

Solve for $y'$:

$$y' = \frac{C}{\sqrt{1 - C^2}}$$

Since $y'$ is constant, the solution is a straight line:

$$y(x) = mx + b$$

where $m$ and $b$ are determined by the boundary conditions.


## The Brachistochrone Problem


:::{note}
ProblemDetermine the curve between two points along which a particle will descend under gravity in the shortest time.
:::

A system is at rest (initially); $y=0$ is a reference level for potential energy: $$\text{kinetic energy} =1/2 mv^2 = 1/2 m \left(\frac{ds}{dt}\right)^2$$ $$\text{potential energy} = -mgy$$

The sum of the two energies is zero initially and therefore zero at any time since the total energy is constant when there is no friction. Hence $$1/2 mv^2 -mgy = 0 \rightarrow v=\sqrt{2gy}$$

Then the integral which we want to minimize is:

$$I = \int dt = \int \frac{ds}{v} = \int \frac{ds}{\sqrt{2gy}} = \int_{x_1}^{x_2} \frac{\sqrt{1 + \left( y' \right)^2}}{\sqrt{2g y}} \, dx$$


## The Brachistochrone Problem (continued)

The Lagrangian is:

$$L(y, y') = \frac{\sqrt{1 + \left( y' \right)^2}}{\sqrt{2g y}}$$

Since $x$ is missing in Lagrangian, we change to $y$ as the integration variable: $$\sqrt{1 + \left( y' \right)^2} dx = \sqrt{1 + \left( y' \right)^2} x' dy = \sqrt{\left( x' \right)^2 + 1}dy$$ $$I = \int \frac{\sqrt{ \left( x' \right)^2} + 1}{\sqrt{2g y}} \, dy = \int L(y, x')$$

Since $\frac{\partial L}{\partial x} = 0$, the Euler-Lagrange equation becomes:

$$\frac{d}{dy} \left( \frac{\partial L}{\partial x'} \right) = \frac{d}{dy} \left (  \frac{x'}{\sqrt{2g y} } \sqrt{\left( x' \right)^2 + 1} \right ) =  0$$


## The Brachistochrone Problem (continued)

The first integral is constant: $$\frac{x'}{\sqrt{2g y} } \sqrt{\left( x' \right)^2 + 1} = \text{const} = \sqrt{c}$$

Solving for $x'$, we get: $$x' = \frac{dx}{dy} = \sqrt{\frac{2gcy}{1-2gcy}}$$

This leads to a differential equation whose solution is a cycloid, given parametrically by:

$$\begin{cases}
x(\theta) = R (\theta - \sin \theta) \\
y(\theta) = R (1 - \cos \theta)
\end{cases}$$

where $R$ is a constant related to the initial conditions.


## The Brachistochrone Problem (continued)


:::{note} Original-slide figure pending review
The original lecture refers to `VC1.png`. The file is preserved in the private Overleaf archive and will be redrawn or published after its reuse rights are verified.
:::

:::{note} Original-slide figure pending review
The original lecture refers to `VC2.png`. The file is preserved in the private Overleaf archive and will be redrawn or published after its reuse rights are verified.
:::

## Generalizations


The Euler-Lagrange equation can be extended to functionals involving higher-order derivatives or multiple functions. For example, for functionals of the form:

$$J[y] = \int_{a}^{b} L(x, y, y', y'') \, dx$$

the Euler-Lagrange equation becomes:

$$\frac{\partial L}{\partial y} - \frac{d}{dx} \left( \frac{\partial L}{\partial y'} \right) + \frac{d^2}{dx^2} \left( \frac{\partial L}{\partial y''} \right) = 0$$


## Variational Principle in Classical Mechanics

Classical mechanics can be formulated through:

- Newton's laws (forces),

- Lagrangian mechanics (energy),

- Variational principle: the path that minimizes the action.

The variational formulation is elegant and generalizes naturally to fields, quantum mechanics, and relativity.


## Action Functional

Define the **Lagrangian** $$L(q, \dot q, t) = T - V,$$ where $T$ is kinetic and $V$ is potential energy.

Define the **action**: $$S[q] = \int_{t_1}^{t_2} L(q, \dot q, t) \, dt.$$

The true path taken by the system satisfies the **principle of stationary action**: $$\delta S = 0.$$


## Euler--Lagrange Equation

Apply a variation: $$q(t) \rightarrow q(t) + \epsilon \, \eta(t), \quad \eta(t_1)=\eta(t_2)=0.$$

Require: $$\frac{d}{d\epsilon} S[q + \epsilon \eta] \bigg|_{\epsilon=0} = 0.$$

This leads to the **Euler--Lagrange equation**: $$\frac{d}{dt} \left( \frac{\partial L}{\partial \dot q} \right)
- \frac{\partial L}{\partial q} = 0.$$

This equation replaces Newton's laws.


## Example: Free Particle

For a particle of mass $m$: $$L = T = \tfrac{1}{2} m \dot q^2.$$

Compute derivatives: $$\begin{align*}
\frac{\partial L}{\partial q} &= 0, \\
\frac{\partial L}{\partial \dot q} &= m \dot q.
\end{align*}$$

Then Euler--Lagrange gives: $$\frac{d}{dt}(m\dot q)=0 \; \Rightarrow \; m\ddot q = 0.$$

The particle moves with constant velocity.


## Example: Harmonic Oscillator

Lagrangian: $$L = \tfrac{1}{2} m \dot q^2 - \tfrac{1}{2} k q^2.$$

Then $$\begin{align*}
\frac{\partial L}{\partial q} &= -k q, \\
\frac{\partial L}{\partial \dot q} &= m \dot q.
\end{align*}$$

Euler--Lagrange equation: $$m \ddot q + k q = 0.$$

We recover the familiar oscillator equation.


## Generalized Coordinates

Lagrangian mechanics allows the use of any convenient coordinates:

- polar coordinates,

- angles in pendulums,

- coordinates adapted to constraints.

Example: simple pendulum (angle $\theta$): $$L = \tfrac{1}{2} m \ell^2 \dot \theta^2 - mg\ell(1 - \cos \theta).$$

Euler--Lagrange equation: $$\ddot\theta + \frac{g}{\ell} \sin \theta = 0.$$


## Advantages of the Variational Method


- Coordinates chosen freely.

- Constraints handled systematically.

- Natural extension to fields and quantum mechanics.

- Symmetries lead directly to conserved quantities (Noether's theorem).


## Noether's Theorem

**Noether's Theorem:** Every continuous symmetry of the action corresponds to a conserved quantity.

If the action is invariant under a transformation $$q \rightarrow q + \epsilon \, \delta q,$$ then there exists a conserved current.


:::{note} Original-slide figure pending review
The original lecture refers to `Noether.png`. The file is preserved in the private Overleaf archive and will be redrawn or published after its reuse rights are verified.
:::

## Noether's Theorem (continued)


- Time Translation Symmetry

  If $L$ has no explicit time dependence: $$\frac{\partial L}{\partial t} = 0,$$ then the transformation $t \rightarrow t + \epsilon$ is a symmetry.

  **Conserved quantity: Energy** $$E = \dot q\frac{\partial L}{\partial \dot q} - L.$$

- Spatial Translation Symmetry $$\frac{\partial L}{\partial x} = 0,$$ then the system is invariant under $x \rightarrow x + \epsilon$.

  **Conserved quantity: Linear Momentum** $$p = \frac{\partial L}{\partial \dot x}.$$


## Noether's Theorem (continued)


- Rotational Symmetry

  If the Lagrangian is invariant under rotations:

  **Conserved quantity: Angular Momentum** $$\mathbf{L} = \mathbf{r} \times m \dot{\mathbf{r}}.$$


## Constraints in Lagrangian Mechanics

Many mechanical systems have constraints, e.g.

- a particle constrained to move on a surface,

- fixed-length pendulum (constraint: $x^2 + y^2 = \ell^2$),

- rolling without slipping.

A constraint can be written as $$f(q_1, q_2, \dots, t) = 0.$$

To enforce the constraint, we introduce a **Lagrange multiplier** $\lambda(t)$.


## Lagrangian With a Constraint

Given an unconstrained Lagrangian $L(q, \dot q)$ and a constraint $f(q,t)=0$, the modified Lagrangian is $$L' = L + \lambda(t) f(q,t).$$

**Euler--Lagrange equations become** $$\frac{d}{dt} \left( \frac{\partial L'}{\partial \dot q_i} \right)
- \frac{\partial L'}{\partial q_i} = 0.$$

The variation with respect to $\lambda$ enforces the constraint: $$\frac{\partial L'}{\partial \lambda} = f(q,t) = 0.$$


## Example: Particle on a Circle

Constraint: particle moves on a circle of radius $R$: $$f(x,y) = x^2 + y^2 - R^2 = 0.$$

Kinetic energy: $$T = \tfrac{1}{2} m (\dot x^2 + \dot y^2).$$

Constrained Lagrangian: $$L' = \tfrac{1}{2} m (\dot x^2 + \dot y^2) + \lambda(x^2 + y^2 - R^2).$$

Variation yields equations of motion and the constraint force term (from $\lambda$).


## Interpretation of the Lagrange Multiplier

The multiplier $\lambda(t)$ corresponds to the **constraint force**.

Example: for a bead constrained to a wire:

- The constraint force keeps the particle on the wire.

- It does no virtual work along allowed displacements.

- Its magnitude is recovered from $\lambda(t)$.

Thus: $\lambda$ is not arbitrary --- it enforces the constraint dynamically.

---

*Migration source: `07_different/variational_calculus.tex` from the archived Overleaf export.*
