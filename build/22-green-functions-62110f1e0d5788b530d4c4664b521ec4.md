...
- The Green's function $G(x,z)$ must satisfy the exact same homogeneous linear differential equation as $y(x)$, but with the forcing term replaced by the Dirac delta function $\delta(x-z)$: 
$$
L G(x, z) = \delta(x-z)
$$
...
- Once the Green's function is known, the solution $y(x)$ to the original inhomogeneous system $Ly(x)=f(x)$ is found by integrating the forcing term $f$ against the Green's function $G(x,z)$: 
$$
...
$$
 with initial conditions $G(0, t') = 0$ and $G'(0, t') = 0$.
...
$$
p^2 \tilde{G}(p, t') - p G(0, t') - G'(0, t') + \\ 2\gamma \left( p \tilde{G}(p, t') - G(0, t') \right) + \omega_0^2 \tilde{G}(p, t') = \mathcal{L}\{\delta(t - t')\}
$$
...
$$
G(t, t') = \frac{1}{\omega_d} e^{-\gamma (t - t')} \sin[\omega_d (t - t')], \quad t' < t
$$
...
$$
y(t) = \frac{1}{\omega_d} \int_0^t e^{-\gamma (t - t')} \sin[\omega_d (t - t')] f(t') \, dt'
$$
