...
The archived sets do not contain a dedicated Lagrange-multiplier problem. The
complete source material remains under
[Assignments and projects](../assignments/index.md); the following problems are
new choices designed for this lecture.
...
1. **Audit a constrained-motion derivation.** Ask ChatGPT to derive the motion
   of a bead on a circle, ellipse, or moving wire using a Lagrange multiplier.
   Check every sign and derivative, compare with a reduced-coordinate or Newtonian
   derivation, and test energy conservation numerically. Interpret the
   multiplier as a physical constraint force.

2. **Design your own constraint.** Choose a smooth curve or surface, use
   ChatGPT to help formulate $\phi(\mathbf q)=0$, and simulate a particle under
   a simple potential. Demonstrate numerically that the trajectory stays on the
   constraint, then explain how $\nabla\phi$ determines the force direction and
   where the method could fail.

3. **The multiplier is not itself the force.** Replace a constraint
   $\phi=0$ by $c\phi=0$ or by another function with the same zero set. Ask
   ChatGPT what should happen to $\lambda$, solve both formulations, and show
   that $\lambda$ changes while $\lambda\nabla\phi$ does not. Explain why the
   physical force must be invariant under a relabeling of the constraint.
