...
:::{note}
Kullback-Leibler (KL) divergence $$I(f,g) = \int f(\mathbf{x}, \beta) \log \biggl[ \frac{f(\mathbf{x}, \beta)}{g(\mathbf{x}, \mu)} \biggr]$$
...
:::{note}
Akaike Information Criterion (AIC) $$AIC = 2K - 2 \log [L(\hat{\mu}|\mathbf{x})]$$
:::
...
:::{note}
Bayesian Information Criterion (AIC) $$BIC = \log(n)K - 2 \log [L(\hat{\mu}|\mathbf{x})]$$
:::
