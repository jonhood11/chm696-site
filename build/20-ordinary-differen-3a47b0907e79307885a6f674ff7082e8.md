...
- classify an ODE by order, linearity, and homogeneity;
- explain the role of a fundamental set of homogeneous solutions;
- use a Wronskian with its required hypotheses; and
...
- On an interval where the normalized coefficients are continuous and the
  leading coefficient does not vanish, the homogeneous solution space of an
  $n$th-order linear ODE has dimension $n$. An initial-value problem therefore
  requires $n$ independent initial data.
...
- If the Wronskian is nonzero at any point, the functions are linearly
  independent. The converse is not true for arbitrary differentiable
  functions: an identically zero Wronskian need not imply dependence. For
  solutions of the same linear homogeneous ODE with continuous coefficients,
  Abel's identity supplies the stronger conclusion used in ODE theory.
...
```
This is one particular solution to the inhomogeneous equation. It depends on
the drive $Q(x)$ and contains no arbitrary constant.
